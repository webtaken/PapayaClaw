...
1. **No availability pre-check** — `GET /v1/server_types?name=<type>` (`locations[].available`) is **not consulted**, neither to skip nor to reorder locations. That flag is unreliable: on 2026-09-21 (~01:30–02:00 UTC) it reported cx23 unavailable in hel1, nbg1 and fsn1 while a direct `POST /servers` for cx23 succeeded in all three in under a second. Commit 23e3ad6 had used it as a gate, so every provision (Polar webhook and staff `POST /api/instances`) failed with `HetznerNoCapacityError` without a single `POST /servers`. `getServerTypeAvailability` still exists for diagnostics/UI only and must never gate provisioning. (The older `datacenters[].server_types.available` field is deprecated by Hetzner; it is not used either.)
2. **Create** — `POST /v1/servers` with `location: <loc>`, walking the preference list in order. If Hetzner answers `412 resource_unavailable` (its out-of-stock code), the next location is tried (`[createServer] <loc> out of stock … trying next location`). Any other HTTP error is rethrown unchanged.
3. **Wait for the action** — a `201` only means *accepted*. The returned `action.id` is polled (`GET /v1/actions/<id>`, every 2 s, up to 90 s) until it leaves `running`. If it ends in `error` (seen 2026-09-21: `resource_unavailable` 22 s after the 201, after which Hetzner removed the server), the phantom server is deleted (404 ignored) and the next location is tried. If it is still `running` after 90 s we proceed and let the SSH poller judge.
4. **Success** — logged as `[createServer] Server <id> created in <loc> (first preference | fallback after <prev>)`. The chosen location is stored on the instance row as `provider_location`.
4. **No stock anywhere** — throws `HetznerNoCapacityError` ("No Hetzner capacity for server type <type> in any configured location (…)") only after every location returned 412. `POST /api/instances` returns it as **503** with that message so the deploy dialog shows it; the Polar webhook logs it under `[Polar Webhook] Provisioning failed for order …`.

Cloudflare tunnel and DNS setup do not depend on the location.

Follow-ups (not implemented): server-type fallback (e.g. `cx23` → `cpx22`) when the whole CX line is out of stock;  persisting failed provisioning attempts so paid-but-unprovisioned orders are visible to staff instead of only in logs.
