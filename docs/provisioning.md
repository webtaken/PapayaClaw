...
2. **Create** — `POST /v1/servers` with `location: <loc>`, walking the preference list in order. If Hetzner answers `412 resource_unavailable` (its out-of-stock code), the next location is tried (`[createServer] <loc> out of stock … trying next location`). That 412 is the **only** signal to move on; any other error is rethrown unchanged.
3. **Success** — logged as `[createServer] Server <id> created in <loc> (first preference | fallback after <prev>: resource_unavailable)`. The chosen location is stored on the instance row as `provider_location`.
